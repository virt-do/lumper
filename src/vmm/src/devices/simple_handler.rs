...
use crate::devices::{RXQ_INDEX, SignalUsedQueue, TXQ_INDEX};
use crate::Tap;
...
// the way queue notification is implemented.
...
impl<M: GuestAddressSpace, S: SignalUsedQueue> SimpleHandler<M, S> {
    ...
    fn write_frame_to_guest(&mut self) -> result::Result<bool, Error> {
        ...
        if count != buf.len() {
            ...
            println!("[WARN] rx frame too large")
        }
    ...
    fn send_frame_from_chain(
        ...
        while let Some(desc) = chain.next() {
            ...
            if len > left {
                println!("[WARN] tx frame too large");
                break;
            }
